...
enum{CMD_CREATE_GRAPH,CMD_GRAPH_IN,CMD_GET_META,CMD_ADD_VERTEX,CMD_ADD_EDGE,CMD_READ_EDGE,CMD_ADD_EDGES,CMD_ADD_VERTEXES,CMD_READ_EDGES,CMD_READ_TWO_EDGES,CMD_READ_EDGE_INDEX,CMD_GET_ALL_VERTEX_NUM,CMD_GET_ALL_EDGE_NUM,CMD_READ_VERTEX,CMD_FLUSH,CMD_GET_ALL_VERTEX,CMD_ADD_SLAVE,CMD_KEEP_SLAVE_STATUS,CMD_GET_INDEX_VERTEX,CMD_GET_INDEX_RANGE_EDGE,CMD_LOST_SLAVES,CMD_LOST_SLAVES_END,CMD_SIZE};//这是命令的类
const string cmd_name[CMD_SIZE]={"create graph","graph is in?","get ip of vertex","add vertex","add edge","read all edge of a vertex","add many edges","add many vertexes","read many edges","read all edge between two vertex","read edges about attribute","get vertex num","get edge num","read vertex","flush","get all vertex","add slave","keep slave status","get some vertex","get range edge","lost slaves","end request of lost slaves"};
